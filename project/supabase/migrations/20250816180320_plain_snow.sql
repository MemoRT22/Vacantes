/*
  # Fix vacancy listing functions

  1. Fixes
    - Remove unnecessary JOINs in COUNT queries that cause GROUP BY issues
    - Simplify queries to avoid aggregation conflicts
    - Maintain all functionality while fixing SQL syntax errors

  2. Functions Updated
    - `list_vacancies_admin`: Fixed COUNT query JOIN issue
    - `list_my_vacancies`: Fixed COUNT query JOIN issue
*/
...
BEGIN
  ...
  -- Get total count (removed unnecessary JOIN)
  SELECT COUNT(*)
  INTO v_total
  FROM vacancies v
